...
class GameControllerTest {
    ...
    void testViewGameById() throws Exception {
        GameResponse.GameResponseBuilder nameResult = GameResponse.builder()
                .count(3L)
        ...
        MockHttpServletRequestBuilder requestBuilder = MockMvcRequestBuilders.get("/games/{id}", 1L);
        MockMvcBuilders.standaloneSetup(gameController)
                .build()
                .perform(requestBuilder)
                .andExpect(MockMvcResultMatchers.status().isOk())
                ...
                .andExpect(MockMvcResultMatchers.content()
                        .string(
                                "{\"gameId\":1,\"name\":\"Name\",\"description\":\"The characteristics of someone or something\",\"price\":2.3,"
    ...
    void testUpdateGameById() throws Exception {
        GameResponse.GameResponseBuilder nameResult = GameResponse.builder()
                ...
                .name("Name");
        GameResponse buildResult = nameResult.price(new BigDecimal("2.3")).build();
        when(gameService.updateGameById(Mockito.<Long>any(), Mockito.<GameRequest>any())).thenReturn(buildResult);
        ...
        gameRequest.setPrice(new BigDecimal("2.3"));
        String content = (new ObjectMapper()).writeValueAsString(gameRequest);
        MockHttpServletRequestBuilder requestBuilder = MockMvcRequestBuilders.put("/games/{id}", 1L)
                .contentType(MediaType.APPLICATION_JSON)
                .content(content);
        MockMvcBuilders.standaloneSetup(gameController)
                .build()
    ...
    void testDeleteGameById() throws Exception {
        ...
        MockHttpServletRequestBuilder requestBuilder = MockMvcRequestBuilders.delete("/games/{id}", 1L);
        MockMvcBuilders.standaloneSetup(gameController)
                .build()
                .perform(requestBuilder)
                .andExpect(MockMvcResultMatchers.status().isOk())
    ...
    void testAddGameToCart() throws Exception {
        ...
        when(gameService.addGameToCart(Mockito.<Long>any(), Mockito.<Long>any())).thenReturn(buildResult);
        MockHttpServletRequestBuilder postResult = MockMvcRequestBuilders.post("/games/{gameId}/create", 1L);
        MockHttpServletRequestBuilder requestBuilder = postResult.param("userId", String.valueOf(1L));
        ResultActions actualPerformResult = MockMvcBuilders.standaloneSetup(gameController).build().perform(requestBuilder);
    ...
    void testRemoveGameFromCart() throws Exception {
        doNothing().when(gameService).removeGameFromCart(Mockito.<Long>any(), Mockito.<Long>any());
        MockHttpServletRequestBuilder deleteResult = MockMvcRequestBuilders.delete("/games/carts/{gameId}", 1L);
        ...
        MockMvcBuilders.standaloneSetup(gameController)
                ...
                .andExpect(MockMvcResultMatchers.status().isOk())
                .andExpect(MockMvcResultMatchers.content().contentType("text/plain;charset=ISO-8859-1"))
                .andExpect(MockMvcResultMatchers.content().string("Game successfully removed from the cart."));
    }
